...
//        sSpinner.setVisible(false);
	...
	private void DeviceBoxActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (deviceBox.getSelectedItem() != null) {
			...
			gdevice = upnpService.getRegistry().getRemoteDevice((UDN) deviceBox.getSelectedItem(),
...
//            System.out.println(gservice);
		...
		if (argumentBox.getSelectedItem() != null) {
			...
			if (gargument != null) {
				...
				if (argumentClass.equals(BooleanDatatype.class)) {
					...
					if (argumentClass.equals(DateTimeDatatype.class) || argumentClass.equals(StringDatatype.class)
	...
	private void CallActionButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (gargument != null) {
			...
			else if (argumentClass.equals(DateTimeDatatype.class) || argumentClass.equals(StringDatatype.class)
					|| argumentClass.equals(CharacterDatatype.class))
		...
		System.out.println("Params:\n    " + upnpService + "\n    " + gservice + "\n    " + actionName + "\n    "
				+ argumentName + "\n    " + value);
		executeAction(upnpService, gservice, actionName, argumentName, value);
	}
...
//                                    System.out.println("co action "+action.toString());                                    
									for (ActionArgument argument : action.getInputArguments()) {
										if (((DefaultComboBoxModel) dVariableBox.getModel())
												.getIndexOf(argument.getRelatedStateVariableName()) == -1) {
		...
		if ((sSpinner.isVisible() || sCheckBox.isVisible() || sTextField.isVisible())
			...
			if (listMap.indexOf(map) < 0) {
				...
				for (Map<String, Object> map1 : listMap) {
					string += "IF (" + map1.get("sDeviceType") + "," + map1.get("sServiceType") + ","
							+ map1.get("sStateVariable") + "==" + map1.get("sValue") + ") THEN ("
							+ map1.get("dDeviceType") + "," + map1.get("dServiceType") + ","
							+ map1.get("dStateVariable") + "==" + map1.get("dValue") + ")\n";
				}
...
//                            throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
						...
						public void eventReceived(GENASubscription sub) {//
							...
							System.out.println("Event number " + sub.getCurrentSequence().getValue() + " from "
...
//                                                System.out.println("co thay device "+remoteDevice.toString());
												...
												for (RemoteService findService : remoteDevice.findServices()) {
													if (findService.getServiceType().getType()
															.equals(next.get("dServiceType").toString())) {
												...
												if (tmpService != null) {// int count=0;
													for (Action action : tmpService.getActions()) {
														for (ActionArgument argument : action.getInputArguments()) {
															if (argument.getRelatedStateVariableName()
																	.equals(next.get("dStateVariable").toString())) {//
																executeAction(upnpService, tmpService, action.getName(),
																		argument.getName(), next.get("dValue"));
															} else
...
//                            throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
	...
	private void executeAction(UpnpService upnpService, Service service, String actionName, String agrumentName,
			Object value) {
...
//                        System.out.println("Not need argument, Params OK");
		...
		if (paramsOk) {
			ActionInvocation invocation = new CallAction(service, actionName, agrumentName, value);
...
//                                System.out.println("Successfully called action!");
					...
					for (ActionArgumentValue actionArgumentValue : invocation.getOutput()) {
						resultArea.append("\nResult( " + currentTime() + " ):\n   "
								+ actionArgumentValue.getArgument().getName() + ": " + actionArgumentValue);
